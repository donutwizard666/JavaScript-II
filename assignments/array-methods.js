...
runners.map(function(items){
  firstNamesAllCaps.push(items.first_name.toUpperCase())
});
...
runners.filter(function(items){
  ...
  return runnersLargeSizeShirt.push(items);
});
...
runners.reduce((total, currentValue) => {
  return ticketPriceTotal += currentValue.donation;
}, 0);
...
// Problem 1 - Small shirts aren't available. Filter the list for everyone that needs a small shirt. 
let runnersSmallSizeShirt = [];

runners.filter(function(items){

  if (items.shirt_size === 'S')
  return runnersSmallSizeShirt.push(items);
});
console.log(runnersSmallSizeShirt);

// Problem 2 - Director wants runners LAST NAMES in uppercase. 
let lastNamesAllCaps = [];

runners.map(function(items){
  lastNamesAllCaps.push(items.last_name.toUpperCase())
});

console.log(lastNamesAllCaps);


// Problem 3 - Director needs first name and email because he is needy
let nameEmail = [];

runners.forEach(function(i){
 return nameEmail.push(`${i.first_name} ${i.email}`);
});
